...
**Saa JP, Tse T, Koh G, Yap P, Baum C, Uribe-Rivera D, Windecker S, Ma H, Davis S, Donnan G, Carey T**. "Trajectory and factors impacting cognitive recovery in the first year after stroke", (submitted to *PLOS One*).
...
[![Launch Rstudio Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/jpsaa/saa_cog_recovery_2023/master?urlpath=rstudio)
...
```
git clone https://github.com/jpsaa/saa_cog_recovery_2023.git
```

Then fetch the container:

```
docker pull jpsaa/saa_cog_recovery_2023
```
...
```
docker run --user root -v $(pwd):/home/rstudio/ -p 8787:8787 -e DISABLE_AUTH=true jpsaa/saa_cog_recovery_2023
```
...
```
docker build -t jpsaa/saa_cog_recovery_2023.
```

and was then pushed to [dockerhub](https://cloud.docker.com/u/smwindecker/repository/docker/jpsaa/saa_cog_recovery_2023). The image used by binder builds off this container, adding extra features needed by binder, as described in [rocker/binder](https://hub.docker.com/r/rocker/binder/dockerfile).
